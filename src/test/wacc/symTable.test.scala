...
class symTableTest extends AnyFlatSpec with BeforeAndAfterEach {
    ...
    var symTable: SymTable = new SymTable(None, None, false)

    override protected def beforeEach(): Unit = {
        symTable = new SymTable(None, None, false)
    }                                                                                                
    ...
    it should "recognise a variable from an enclosing scope" in {
        val symTable2 = new SymTable(Some(symTable), None, false)
    ...
    it should "be able to store and return a single function" in {
        symTable.varDefinedGlobal("gta") shouldBe false
        symTable.addSymbol("gta", FUNCTION(S_INT)(new SymTable(Some(symTable), None, false))) shouldBe true
        symTable.funDefinedLocal("gta") shouldBe true
        symTable.funDefinedGlobal("gta") shouldBe true
        symTable.findFunGlobal("gta") shouldBe Some(FUNCTION(S_INT)(new SymTable(Some(symTable), None, false)))
        symTable.findFunLocal("gta") shouldBe Some(FUNCTION(S_INT)(new SymTable(Some(symTable), None, false)))
    }

    it should "allow an identifier to be used only once" in {
        symTable.varDefinedGlobal("gta") shouldBe false
        symTable.addSymbol("gta", FUNCTION(S_INT)(new SymTable(Some(symTable), None, false))) shouldBe true
        symTable.addSymbol("gta", FUNCTION(S_STRING)(new SymTable(Some(symTable), None, false))) shouldBe false
        symTable.findFunGlobal("gta") shouldBe Some(FUNCTION(S_INT)(new SymTable(Some(symTable), None, false)))
        symTable.findFunLocal("gta") shouldBe Some(FUNCTION(S_INT)(new SymTable(Some(symTable), None, false)))
    }
    ...
    it should "recognise a function from an enclosing scope" in {
        val symTable2 = new SymTable(Some(symTable), None, false)
        symTable.addSymbol("gta", FUNCTION(S_INT)(new SymTable(Some(symTable), None, false)))
    ...
    it should "shadow a funcion from an enclosing scope" in {
        ...
        symTable2.addSymbol("gta", FUNCTION(S_STRING)(new SymTable(Some(symTable), None, false))) shouldBe true
        symTable2.findFunGlobal("gta") shouldBe Some(FUNCTION(S_STRING)(new SymTable(Some(symTable), None, false)))
        symTable2.findFunLocal("gta") shouldBe Some(FUNCTION(S_STRING)(new SymTable(Some(symTable), None, false)))
        symTable.findFunGlobal("gta") shouldBe Some(FUNCTION(S_INT)(new SymTable(Some(symTable), None, false)))
        symTable.findFunLocal("gta") shouldBe Some(FUNCTION(S_INT)(new SymTable(Some(symTable), None, false)))
        //TODO: KEY CONSIDERATION! DOES THAT APPLY TO FUNCTIONS?
    }
    ...
    "The position assign functionality" should "assign the correct positions" in {
        ...
        symTable.assignPositions(new Aarch64_formatter) shouldBe -80
        ...
        symTable.findVarGlobal("par9").get.pos  shouldBe OnStack(16)
        symTable.findVarGlobal("par10").get.pos shouldBe OnStack(32)
        symTable.findVarGlobal("par11").get.pos shouldBe OnStack(48)
        symTable.findVarGlobal("par12").get.pos shouldBe OnStack(64)
        symTable.findVarGlobal("par13").get.pos shouldBe OnStack(80)
        symTable.findVarGlobal("par14").get.pos shouldBe OnStack(96)
        ...
        symTable.findVarGlobal("var11").get.pos shouldBe OnStack(-16)
        symTable.findVarGlobal("var12").get.pos shouldBe OnStack(-32)
        symTable.findVarGlobal("var13").get.pos shouldBe OnStack(-48)
        symTable.findVarGlobal("var14").get.pos shouldBe OnStack(-64)
    }
