...
class aarch64FormTest extends AnyFlatSpec {
    ...
    "The arm64 formatter" should "process a simple mov command with registers" in {
        aarch64_formatter.generateAssembly(List(Move(Register(1), Register(2)))) shouldBe ".align 4\n.text\n.global main\nmov\tX2, X1\n"
    }

    it should "process a simple mov with an immediate" in {
        aarch64_formatter.generateAssembly(List(Move(ImmNum(1), Register(2)))) shouldBe ".align 4\n.text\n.global main\nmov\tX2, #1\n"
    }

    it should "process an add" in {
        aarch64_formatter.generateAssembly(List(AddI(ImmNum(1), Register(2)))) shouldBe ".align 4\n.text\n.global main\nadd\tX2, X2, #1\n"
    }
