...
class aarch64FormTest extends AnyFlatSpec {
    ...
    "The arm64 formatter" should "process a simple mov command with registers" in {
        aarch64_formatter.generateAssembly(List(Move(Register(1), Register(2)))) shouldBe "mov\tX2, X1\n"
    }

    it should "process a simple mov with an immediate" in {
        aarch64_formatter.generateAssembly(List(Move(ImmNum(1), Register(2)))) shouldBe "mov\tX2, #1\n"
    }

    it should "process an add" in {
        aarch64_formatter.generateAssembly(List(AddI(ImmNum(1), Register(2)))) shouldBe "add\tX2, X2, #1\n"
    }

    it should "add error checking for division" in {
        aarch64_formatter.generateAssembly(List(DivI(ImmNum(0), Register(2)))) shouldBe 
        "cmp\tXZR, #0\nb.eq\t_errDivZero\nsdiv\tX2, X2, #0\n" + """
